...
```{r}
ftable(xtabs(~ crop + trat_id + sitio, cober_dat))
...
lmer_T6  <- with(asin_tran, 
                lmer(linkfun(val) ~ trat_id + (1|sitio), 
                     data = cober_T6))
plot(lmer_T6)
car::Anova(lmer_T6)
...
```{r}
cober_T6 %>% 
  mutate(regional = stringr::str_to_upper(sitio)) %>% 
  ggplot() + 
  aes(x=trat_id, y= val)+
  facet_grid(.~sitio)+
...
cober_dat %>% 
  ...
  filter(crop=="T") %>%
  select(-sitio) %>% 
  left_join(cober_dat, multiple = "all") -> cober_T2
```
