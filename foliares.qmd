...
eval %>% 
  ...
  select(MR_inc=response, limite_inferior=lower.CL, limite_superior=upper.CL, Tukey=.group, `%Control`) %>% 
  as_hux() %>% 
  set_font_size(10) %>% 
  theme_article() %>% 
  set_tb_padding(.1) %>% 
...
> En Tandil no se observaron diferencias significativas entre los tratamientos. Vale recordar la misa consideracion que en el analisis de rendimiento de los ensayos de curasemillas: las respuestas medias de varios tratamientos superaron ampliamente los costos de su adopción. 
